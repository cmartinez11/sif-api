<table class="w-full text-sm text-left">
    <thead class="bg-[#0CC954] text-white">
        <tr>
            ...
            <th class="px-2 py-2 text-center">CANTIDAD DE MILL. X FARDO</th>
            <th class="px-2 py-2 text-center">TOTAL DE FARDOS</th>
            <th class="px-2 py-2 text-center">TOTAL DE MILLARES</th>
    ...
    <tbody>
        <template x-for="(item, index) in items" :key="index">
            <tr x-show="!item.oculto" class="border-b" :class="item.estado_item === 'Rechazado' ? 'bg-orange-50 opacity-75' : (index % 2 === 0 ? 'bg-white' : 'bg-gray-50')">
                ...
                <td class="px-2 py-2">
                    <input type="number" x-model.number="item.cantidad" @input="calculateRow(index)" class="w-full text-right text-xs border-gray-300 rounded">
                </td>
                <td class="px-2 py-2">
                ...
                <td class="px-2 py-2 text-right font-medium">
                    <span x-text="Number(item.total_kilos || 0).toFixed(2)"></span>
                </td>
                <td class="px-2 py-2">
