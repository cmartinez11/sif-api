                ...
                <form action="{{ route('clientes.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                        ...
                        <div class="md:col-span-2 lg:col-span-3">
                            ...
                            <input type="text" name="direccion" value="{{ old('direccion') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-fenix-green focus:ring focus:ring-fenix-green focus:ring-opacity-50 py-2 md:py-1.5">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Departamento</label>
                            <input type="text" name="departamento" value="{{ old('departamento') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-fenix-green focus:ring focus:ring-fenix-green focus:ring-opacity-50">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="distrito" value="{{ old('distrito') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-fenix-green focus:ring focus:ring-fenix-green focus:ring-opacity-50">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Condición de Pago</label>
                            <select name="condicion_pago" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-fenix-green focus:ring focus:ring-fenix-green focus:ring-opacity-50">
                                <option value="CONTADO" {{ old('condicion_pago', 'CONTADO') === 'CONTADO' ? 'selected' : '' }}>CONTADO</option>
                                <option value="7 DIAS" {{ old('condicion_pago', 'CONTADO') === '7 DIAS' ? 'selected' : '' }}>7 DIAS</option>
                                <option value="10 DIAS" {{ old('condicion_pago', 'CONTADO') === '10 DIAS' ? 'selected' : '' }}>10 DIAS</option>
                                <option value="15 DIAS" {{ old('condicion_pago', 'CONTADO') === '15 DIAS' ? 'selected' : '' }}>15 DIAS</option>
                                <option value="20 DIAS" {{ old('condicion_pago', 'CONTADO') === '20 DIAS' ? 'selected' : '' }}>20 DIAS</option>
                                <option value="30 DIAS" {{ old('condicion_pago', 'CONTADO') === '30 DIAS' ? 'selected' : '' }}>30 DIAS</option>
                                <option value="45 DIAS" {{ old('condicion_pago', 'CONTADO') === '45 DIAS' ? 'selected' : '' }}>45 DIAS</option>
                                <option value="60 DIAS" {{ old('condicion_pago', 'CONTADO') === '60 DIAS' ? 'selected' : '' }}>60 DIAS</option>
                                <option value="90 DIAS" {{ old('condicion_pago', 'CONTADO') === '90 DIAS' ? 'selected' : '' }}>90 DIAS</option>
                            </select>
                        </div>
                        <div class="md:col-span-2 lg:col-span-3">
                            <label class="block text-sm font-medium text-gray-700">Asociar a Contacto (Opcional)</label>
                            <select name="contacto_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-fenix-green focus:ring focus:ring-fenix-green focus:ring-opacity-50 py-2 md:py-1.5">
                                <option value="">- Seleccionar Contacto -</option>
                                @foreach($contactos as $con)
                                    <option value="{{ $con->id }}" {{ old('contacto_id') == $con->id ? 'selected' : '' }}>
                    ...
                    <div class="mt-4 border border-gray-200 rounded-lg shadow-sm overflow-hidden bg-white" 
                         x-data="{ 
                            pico: '{{ old('tipo_preforma', '') }}',
                            gramaje: '{{ old('gramaje', '') }}',
                            soplado: '{{ old('aplicacion', '') }}',
                            tabla: {
                                'PICO 1881': {
                                    '12.4 GR': '300 ML - 330 ML',
                                    '12.8 GR': '300 ML - 350 ML',
                                    '14.5 GR': '400 ML - 450 ML',
                                    '17 GR': '625 ML',
                                    '17.7 GR': '650 ML',
                                    '19.7 GR': '900 ML',
                                    '23 GR': '900 ML - 1LT',
                                ...
                                'PICO 48 MM': {
                                    '60 GR - 48 MM': '3 LT',
                                    '90 GR - 48 MM': '4 LT - 7 LT',
                                    '100 GR - 48 MM': '7 LT - 10 LT',
                                    '106 GR - 48 MM': '10 LT'
                                },
                                'PICO 2621': {
                                    '9 GR - ACEITE': '250 ML',
                                    '15 GR - ACEITE': '500 ML',
                                    '23 GR - ACEITE': '900 ML - 1LT'
                                }
                            },
                        ...
                        <div class="p-3 space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="text" name="proveedor_actual" value="{{ old('proveedor_actual') }}" placeholder="Nombre del competidor" class="block w-full rounded border-gray-300 shadow-sm focus:border-fenix-green focus:ring-fenix-green text-xs py-2 md:py-1 h-10 md:h-8">
                                </div>
                                <div>
                                    ...
                                    <select name="problemas_proveedor" class="block w-full rounded border-gray-300 shadow-sm focus:border-fenix-green focus:ring-fenix-green text-xs py-2 md:py-1 h-10 md:h-8">
                                        ...
                                        <option value="Precio" {{ old('problemas_proveedor') === 'Precio' ? 'selected' : '' }}>Precio</option>
                                        <option value="Calidad" {{ old('problemas_proveedor') === 'Calidad' ? 'selected' : '' }}>Calidad</option>
                                        <option value="Entrega" {{ old('problemas_proveedor') === 'Entrega' ? 'selected' : '' }}>Entrega</option>
                                        <option value="Stock" {{ old('problemas_proveedor') === 'Stock' ? 'selected' : '' }}>Stock</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 items-end">
                                <div>
                                    ...
                                    <input type="number" name="cant_maquinas" value="{{ old('cant_maquinas', 0) }}" class="block w-full rounded border-gray-300 shadow-sm focus:border-fenix-green focus:ring-fenix-green text-xs py-2 md:py-1 h-10 md:h-8">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">Vol. Mensual</label>
                                    <input type="number" step="0.01" name="vol_mensual" value="{{ old('vol_mensual', 0) }}" class="block w-full rounded border-gray-300 shadow-sm focus:border-fenix-green focus:ring-fenix-green text-xs py-2 md:py-1 h-10 md:h-8">
                                </div>
                                    ...
                                    <div class="flex flex-col sm:flex-row items-center gap-2">
                                        <div class="w-full sm:flex-1">
                                            ...
                                            <select name="frecuencia_compra" class="block w-full rounded border-gray-300 shadow-sm focus:border-fenix-green focus:ring-fenix-green text-xs py-2 md:py-1 h-10 md:h-8">
                                                ...
                                                <option value="Semanal" {{ old('frecuencia_compra') === 'Semanal' ? 'selected' : '' }}>Semanal</option>
                                                <option value="Quincenal" {{ old('frecuencia_compra') === 'Quincenal' ? 'selected' : '' }}>Quincenal</option>
                                                <option value="Mensual" {{ old('frecuencia_compra') === 'Mensual' ? 'selected' : '' }}>Mensual</option>
                                                <option value="Trimestral" {{ old('frecuencia_compra') === 'Trimestral' ? 'selected' : '' }}>Trimestral</option>
                                            </select>
                                        </div>
                                        <div class="w-full sm:w-auto flex flex-col">
                                            ...
                                            <div class="flex items-center justify-center bg-white border border-gray-300 px-4 h-10 md:h-8 rounded shadow-sm">
                                                ...
                                                <input type="checkbox" name="urgencias_frecuentes" id="urgencias_frecuentes" value="1" {{ old('urgencias_frecuentes') ? 'checked' : '' }} class="rounded border-gray-300 text-fenix-green h-5 w-5 md:h-4 md:w-4">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <!-- Observaciones -->
                            <div>
                                <textarea name="observaciones" rows="2" placeholder="Observaciones estratégicas sobre el perfil técnico..." class="block w-full rounded border-gray-300 shadow-sm focus:border-fenix-green focus:ring-fenix-green text-xs py-1">{{ old('observaciones') }}</textarea>
                            </div>
                        </div>
                    </div>
                    <div class="mt-6 flex flex-col md:flex-row justify-end gap-3">
                        ...
                        <button type="submit" class="w-full md:w-auto bg-green-500 text-white font-bold py-3 md:py-2 px-6 rounded shadow shadow-lg transition-all">Guardar Cliente</button>
                    </div>
                </form>
            </div>
        </div>
