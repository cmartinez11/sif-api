    ...
    <script>
        function cotizacionForm(initialItems) {
            return {
                ...
                vendedor_campo_id: '{{ $cotizacione->vendedor_campo_id }}',
                observaciones: `{!! $cotizacione->observaciones !!}`,
                allClientes: @json($clientes),
                moneda: '{{ $cotizacione->moneda }}',
                tipo_cambio: '{{ $cotizacione->tipo_cambio ?? 1 }}',
                // Estado del cliente seleccionado para reactividad
                cliente: {
                    nombre: '{{ $cotizacione->cliente->nombre }}',
                    ruc: '{{ $cotizacione->cliente->ruc }}',
                    direccion: '{{ $cotizacione->cliente->direccion }}',
                    condicion_pago: '{{ $cotizacione->cliente->condicion_pago }}',
                    provincia: '{{ $cotizacione->cliente->provincia }}'
                },
                items: initialItems || [],
